  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        console.error(err)
        setMessage({ type: "error", text: "Không thể tải giỏ hàng" })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (payment === "COD") {
        ...
        setMessage({ type: "success", text: "Đặt hàng thành công! Cảm ơn bạn." })
      ...
      if (payment === "VNPAY") {
        ...
        const orderId = orderRes.data.id
        const payRes = await createPayment({ orderId, totalPrice: total })
        window.location.href = payRes.data.paymentUrl
      }
    } catch (err) {
      console.error(err)
      setMessage({ type: "error", text: "Đặt hàng thất bại, vui lòng thử lại" })
      ...
      <div className="px-4 md:px-36 py-10 font-futura-regular min-h-screen">
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-10">
          <div className="space-y-6">
            <div>
              <h2 className="font-futura-regular text-xl mb-3">Thông tin cá nhân</h2>
            ...
            <div>
              <h2 className="font-futura-regular text-xl mb-3">Nơi giao hàng</h2>
              <input
                required
                name="address"
            ...
            <div>
              <h2 className="font-futura-regular text-xl mb-3">Phương thức thanh toán</h2>
              <div className="space-y-3">
          ...
          <div className="border p-5 h-fit">
            <h2 className="font-frankfurter text-xl mb-4">Tóm tắt đơn hàng</h2>

            {loading && <p className="text-sm text-gray-500 mb-4">Đang tải...</p>}

            <div className="space-y-4 mb-4">
              {cartItems.map(item => (
                <div key={item.id} className="flex gap-3">
                  <img src={item.imgMain} className="w-16 h-16 rounded object-cover" alt={item.name} />
